...
Keep the same Reel press held and slide about 32 CSS pixels left/right (15-pixel neutral dead zone). Input remains captured outside the button. The large arrow pulses throughout the Run and changes color when acquired. Reel follows the horizontal pointer displacement, clamped to 16% of the game frame width on either side; the original press remains the drag origin. Release returns it to center. Bass reversals change that arrow under the same held pointer. Release overrides direction immediately. Re-engaging starts a new drag origin.
...
At 100 Tension the line snaps immediately, with a near-snap cue at 96; there is no full-bar grace period. Unheld Reel at zero Catch Progress accumulates a three-second escape timer, with a visible warning after 1.5 seconds. Resuming Reel cancels this timer, and pause freezes it. Giving line while progress remains above zero still uses ordinary progress decay. Escape returns to Ready with the same encounter, without changing the Book.
